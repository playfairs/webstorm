...
export const handleSpam = async (args: Args): Promise<void> => {
  if (!isWebhookUrl(args.url)) {
    exitError('Invalid webhook URL');
  }

  const defaultMsg = await getMessage('spam');
  let payload = buildPayload(args.msg || defaultMsg);

  if (args.json) {
    try {
      const custom = await readJson(args.json);
      payload = buildPayload(args.msg, custom);
    } catch (err) {
  ...
  info(`Spamming ${args.url} with ${threads} threads...`);
